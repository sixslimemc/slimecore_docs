...
While any arbitrary work can be done with entrypoints, their primary purpose is to replace `#minecraft:tick`; this can be done via schedule loops (functions that `/schedule` themselves). Defining a single entrypoint may be sufficient for most datapacks (and is recommended when just starting, with entrypoint ID `main`), but note the benefits of defining multiple highlighted in [this section](./beneficial_practices.md#entrypoint-separation).

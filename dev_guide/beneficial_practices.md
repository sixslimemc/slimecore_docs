...
SlimeCore's loading system provides the framework to achieve this goal, but datapacks' functional compatibility and usability matters just as much, if not more. Even so, SlimeCore intentionally does not try and control such elements, as they are things that *you*, the datapack developer(s), should be in control over.

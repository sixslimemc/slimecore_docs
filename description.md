...
SlimeCore is designed to be **minimal and unobtrusive**. SlimeCore only implements datapack loading, it intentionally does not implement any "frontend" features (chat messages, dialogs, user-facing functions etc.). Instead, it provides a public API for other datapacks to use, such that "frontends" can be created/shared like any other datapack. For instance, A datapack could easily implement chat messages to admins that provide download URL(s) for any missing dependencies upon world reload. This has the added benefit of **customizability**.

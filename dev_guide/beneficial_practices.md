...
Both hooks and [contracts](./full_guide.md#contracts) allow for arbitrary datapack integration, however, the key difference is that hooks provide the option for *any amount* of datapacks to extend behavior, while contracts require *exactly one* datapack to satisfy/implement a behavior.

...
A key advantage of entrypoints is that they can be explicitly ordered against [dependencies'](#dependencies) entrypoints. For instance, if datapack X defines entrypoint `foo`, and depends on datapack Y, which defines entrypoint `bar`, datapack X can specify `foo` to run explicitly before/after `bar`. Given both `foo` and `bar` are schedule loops, this means that `foo` will always run before/after `bar` on any given tick.

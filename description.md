...
- Instead of managing datapacks one-at-a-time via `/datapack` (potentially creating invalid world state(s)), management operations are *staged* via SlimeCore and then either performed all-at-once upon world reload or not-at-all, depending on if they are valid.

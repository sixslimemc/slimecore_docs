...
SlimeCore datapacks are managed (disabled, re-enabled, uninstalled) via the `slimecore:rebuild` function, which explicitly triggers a rebuild with input staged changes. As described, these changes will only take effect if they would result in a valid build, otherwise doing nothing. Managing SlimeCore-loaded datapacks with `/datapack` is improper.

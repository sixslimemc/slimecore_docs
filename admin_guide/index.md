...
SlimeCore is a datapack that *loads other datapacks*. Without going into too much detail, it attempts to garuntee that all SlimeCore-loaded datapacks load correctly and to disallow any changes that would cause them not to. For instance, if datapack A requires datapack B (i.e. B is a dependency of A), SlimeCore will not allow datapack A to load until datapack B is installed and enabled.

...
By default, SlimeCore expects datapack paths (name of file/folder a world's `datapacks/` folder) to follow a specific format *(See [formats](./admin_guide/key_concepts.md#datapack-paths))*. If for whatever reason the path of a SlimeCore-loaded datapack doesn't or can't follow this format, SlimeCore provides a method to manually link a datapack to its path via ad hoc in-game commands.

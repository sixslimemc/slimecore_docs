...
The logs contains a list of all datapacks enabled (i.e. in the [current build](#builds)), as well as their entrypoints, in the order that they are loaded/called. (Entrypoints are likely only useful for datapack development and can be disregarded for adminstration purposes.)

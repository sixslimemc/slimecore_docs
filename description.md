...
A rebuild can also be manually initiated via the `slimecore:rebuild` function. Inputs can be provided to this function to "stage" datapacks for disabling, enabling, or uninstallation. If the staged changes would result in an invalid build (i.e. the rebuild fails), no changes to the world are actually made. This is the proper way to enable, disable, and uninstall SlimeCore-loaded datapacks.

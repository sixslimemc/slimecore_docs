...
When a rebuild *succeeds*, the appropriate changes to datapack loading and [build data](#build-data) are made.
...
| `aux.contract_map` | `{<pack_id...>: {<contract id...>: PackManifest}}` | (Auxilary) Struct where the key-path `<contract's pack_id>.<contract id>` contains the pack manifest of the pack that satisfies the respective contract. |

...
Outside of command output (e.g. output from [info functions](#info-functions)), SCDev will only send chat messages to players with the `scdev.listener` tag. Many chat messages sent by SCDev contain clickable/hoverable elements, though click actions will only work if the player has operator privilages.
...
**SlimeCore Info:** \
The function `scdev:-/info/slimecore` displays information about SlimeCore itself and does not take any input.

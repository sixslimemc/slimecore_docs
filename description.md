...
vai a *manifest* system. Datapacks define their manifests, and upon world reload, SlimeCore processes all datapacks' manifests, validates relationships, then executes a compatible load/calling order if everything is valid.
...
SlimeCore intentionally does not implement any "front-end" features such as chat messages, dialogs, or user-functions. Instead, it provides a public API and functions *hooks* that other datapacks can use, such that SlimeCore "front-ends" are customizable and decentralized. For example, a datapack can subscribe to `#slimecore:hook/meta_info/rebuild/end` and display a message to admins containing the download URL(s) for any missing dependencies (dependency download URLs are provided by dependent's manifest) found after a rebuild.

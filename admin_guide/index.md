...
In all practical cases, you should install a **frontend** datapack. SlimeCore does not include any UI on its own; a frontend is expected to provide such. Frontends are developed and documented independently of SlimeCore like any other datapack; your chosen frontend should provide its own documentation on its own specific usage--this guide, however, is intended to be frontend-agnostic.
...
> While SlimeCore is still in an early-adoption phase, there may not be a selection of frontends. However, if you find the current selection to be insufficient, it is not too difficult to [create your own](../interface_guide/index.md).

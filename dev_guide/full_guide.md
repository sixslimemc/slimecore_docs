...
SlimeCore ensures that all required dependencies are installed/enabled and that all installed dependencies (optional or not) are of a compatible version (see below) before your datapack loads at all. Your datapack will always [load](#load-tag) **after** all of it's dependencies.

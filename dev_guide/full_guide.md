...
Below are a handful of tips, guidelines, and practices that are not strictly defined or required by SlimeCore, but may aide in maximizing a datapack's compatibility and/or usability.
...
Examples of effective namespacing within a datapack with pack ID `foo` would be:
- Only using NBT storage locations that start with `foo:`.
...
For in-game artifacts, one approach is to prefix identifiers with `_` if they are meant to be private, similar to [namespacing](#namespacing), (`_` in particular works well alongside namespacing because pack IDs cannot start with `_`).

...
A common use of entrypoints is to replace `#minecraft:tick` by defining a single entrypoint that starts a `/schedule` tick loop. *A function that calls `/schedule <self> 1t` will run itself every tick.*

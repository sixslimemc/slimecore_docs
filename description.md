...
Instead of adding functions to `#minecraft:load` and `#minecraft:tick`, datapacks include a special *manifest* function that includes it's entrypoints (as well as the information listed above). Upon world reload, SlimeCore processes all datapacks' manifests, validates relationships, then executes a compatible load/calling order if everything is valid. 
